...
function addFlavor(arrayName,flavorName){
   arrayName.unshift(flavorName);
   return arrayName;
}
...
function removeLastFlavor(anArr){
   anArr.pop();
   return anArr;
}
...
function filterByWord(iceCreamList, flavor){
    let specificFlavors=[]
    for (let i=0; i < iceCreamList.length;i++){
        if(iceCreamList[i].includes(flavor)){
            specificFlavors.push(iceCreamList[i]);
        }
        else{
            continue;
        }
    }return specificFlavors;
}
